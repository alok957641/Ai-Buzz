...
export default function Contact() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      alert("Server connect nahi ho raha bhai! Check karo backend live hai ya nahi. ❌");
    } finally {
    ...
    <section id="contact" className="relative bg-black py-28 overflow-hidden">
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-6">
        <div className="text-center mb-16">
          <p className="inline-block mb-4 px-4 py-1 text-sm rounded-full bg-white/10 text-gray-300 backdrop-blur-md">📩 Contact Us</p>
          <h2 className="text-3xl sm:text-5xl font-extrabold text-white">
            Let’s Build Something <span className="block bg-gradient-to-r from-pink-500 to-blue-500 bg-clip-text text-transparent">Amazing Together</span>
          </h2>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-12 items-center">
          <div className="space-y-6">
            ...
            <a href="https://wa.me/919113709625" className="inline-block px-6 py-4 rounded-full bg-green-500 text-white font-bold">💬 Chat on WhatsApp</a>
          </div>
          ...
          <form onSubmit={handleSubmit} className="bg-white/5 border border-white/10 rounded-3xl p-8 space-y-6">
            ...
            <button
              ...
              className={`w-full py-4 rounded-full font-bold text-white flex items-center justify-center gap-3 transition-all ${
                loading ? "bg-gray-600 cursor-wait" : "bg-gradient-to-r from-pink-500 to-purple-600 hover:scale-[1.02]"
              }`}
            >
              {loading ? (
                <>
